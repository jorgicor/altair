...
BOOTSEC bootsektor =   {0x7E18,     // Skok na systemovy zavadzac
                        0x80,       // 0x80 - nepouzite
                        0x02,       // 1. znacka MB-02
                        0x0052,     // Pocet fyzickych stop na disku (82)
                        0x000B,     // Pocet sektorov na stope (11)
...
int main(int argc,char *argv[])
{
 ...
 if((numofdir == 0xFFFF) || (numofdir >= 256)){   // 256 dirs are maximum !!!
...
BYTE checkimage (void)        // Check existing image is MB-02+ image
{
 ...
 if(retval == 0){             // if it is MB-02+ image, check his params
   if ((bootsektor.numofcyl == 0x0052) && (bootsektor.numofsec == 0x000B)
      && (bootsektor.numofhead == 0x0002) && (bootsektor.secpercluster == 0x0001)){
     retval = 0;                // We know work with this image
   } else {
     retval = 2;                // Different image, not supported ( yet ;)
   }
 }
 return(retval);
}
